...
public class MediaAppWidgetProvider extends AppWidgetProvider {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(UPDATE_WIDGET_ACTION) && hasInstances(context)) {
            ...
            views.setTextViewText(R.id.artist_album, context.getText(R.string.widget_initial_text));
            ...
            views.setImageViewResource(R.id.mode, R.drawable.playmode_loop);
            views.setImageViewResource(R.id.low_volume, R.drawable.volume_low);
            views.setImageViewResource(R.id.high_volume, R.drawable.volume_high);
    ...
    private void defaultAppWidget(Context context, int[] appWidgetIds) {
        ...
        views.setViewVisibility(R.id.trackname, View.GONE);
        views.setTextViewText(R.id.artist_album, res.getText(R.string.widget_initial_text));
        //views.setViewVisibility(R.id.album, View.GONE);
    ...
    void performUpdate(MediaPlaybackService service, int[] appWidgetIds) {
        ...
        if (errorState != null) {
            ...
            views.setViewVisibility(R.id.trackname, View.GONE);
            views.setTextViewText(R.id.artist_album, errorState);
            //views.setViewVisibility(R.id.album, View.GONE);
            ...
            views.setTextViewText(R.id.artist_album, artistName + "——" + albumName);
        ...
        if (playing) {
            ...
            if (titleName == null && !mPauseState && (errorState == null)) {
                views.setViewVisibility(R.id.trackname, View.GONE);
                views.setTextViewText(R.id.artist_album, res.getText(R.string.emptyplaylist));
                //views.setViewVisibility(R.id.album, View.GONE);
            }
            views.setImageViewResource(R.id.pause, R.drawable.notification_play);
        }
        ...
        if (repeatMode == MediaPlaybackService.REPEAT_ALL) {
            ...
            views.setImageViewResource(R.id.mode, R.drawable.playmode_one);

        } else if (shuffleMode == MediaPlaybackService.SHUFFLE_NORMAL) {
            views.setImageViewResource(R.id.mode, R.drawable.playmode_random);

        }

        Log.i("Music_MediaAppWidgetProvider", "performUpdate():change the playmode icon");

        String currenttimer = MusicUtils.makeTimeString(service, service.position() / 1000);
        String totaltimer = MusicUtils.makeTimeString(service, service.duration() / 1000);
        int progress = (int) (1000 * service.position() / service.duration());
        String endtimer = MusicUtils.makeTimeString(service, (service.duration() - service.position()) / 1000);

        views.setTextViewText(R.id.currenttimer, currenttimer);
        views.setTextViewText(R.id.totaltimer, "-"+endtimer);
        views.setProgressBar(R.id.progress, 1000, progress, false);

       /* Log.i("Music_", "currenttimer : " + currenttimer);
        Log.i("Music_", "totaltimer : " + totaltimer);
        Log.i("Music_", "currentProgress : " + progress);*/


        AudioManager audioManager = (AudioManager) service.getSystemService(Context.AUDIO_SERVICE);
        int streamMaxVolume = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
        int streamVolume = audioManager.getStreamVolume(AudioManager.STREAM_MUSIC);
        views.setProgressBar(R.id.progress_volume, streamMaxVolume, streamVolume, false);

        /*Log.i("Music_", "streamMaxVolume: " + streamMaxVolume);
        Log.i("Music_", "streamVolume: " + streamVolume);*/
    ...
    private void linkButtons(Context context, RemoteViews views, boolean playerActive) {
        ...
        intent = new Intent(MediaPlaybackService.TOGGLEMODE_ACTION);
        intent.setComponent(serviceName);
        pendingIntent = PendingIntent.getService(context,
                0 /* no requestCode */, intent, 0 /* no flags */);
